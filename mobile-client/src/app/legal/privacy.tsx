...
export default function PrivacyScreen() {
  ...
  const sections = ['controller', 'data', 'purposes', 'bases', 'sharing', 'retention', 'rights', 'security', 'contact'];
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Text style={styles.title}>{t('legal.privacyTitle')}</Text><Text style={styles.version}>{t('legal.version')}</Text>
    {sections.map((section) => <View key={section} style={styles.section}><Text style={styles.heading}>{t(`legal.privacy.${section}Title`)}</Text><Text style={styles.body}>{t(`legal.privacy.${section}Body`)}</Text></View>)}
