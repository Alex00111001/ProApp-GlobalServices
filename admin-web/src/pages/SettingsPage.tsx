...
export function SettingsPage() {
  ...
  const decide = useMutation({ mutationFn: () => review?.decision === 'approve'
    ? api(`/v1/admin/role-change-requests/${review.id}/approve`, { method: 'POST', body: JSON.stringify({ reason: reviewReason }) })
    : api(`/v1/admin/role-change-requests/${review?.id}/reject`, { method: 'POST', body: JSON.stringify({ reason: reviewReason }) }), onSuccess: async () => { setReview(null); setReviewReason(''); await queryClient.invalidateQueries({ queryKey: ['admin-role-requests'] }); await queryClient.invalidateQueries({ queryKey: ['admin-roles'] }) } })
