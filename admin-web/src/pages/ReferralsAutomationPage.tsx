...
function usePage(statuses: string[] = []) {
  const [page, setPage] = useState(1)
  const [status, setStatus] = useState('')
  const params = new URLSearchParams({ page: String(page), limit: '25' })
  if (status) params.set('status', status)
  const filters = statuses.length ? <div className="filters"><label>Estado<select aria-label="Filtrar por estado" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1) }}><option value="">Todos</option>{statuses.map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div> : null
  return { page, setPage, status, query: params.toString(), filters }
}

function ProgramsPanel() {
  const state = usePage(['DRAFT', 'SCHEDULED', 'ACTIVE', 'PAUSED', 'COMPLETED', 'ARCHIVED'])
  const query = useQuery({ queryKey: ['f8-programs', state.query], queryFn: () => api(`/v1/admin/referrals/programs?${state.query}`, { schema: referralProgramListSchema }) })
  return <>{state.filters}<QueryState loading={query.isLoading} error={query.error} empty={!query.data?.items.length}>{query.data && <article className="panel table-panel"><div className="table-scroll"><table><thead><tr><th>Programa</th><th>Estado</th><th>Versión</th><th>Mercados</th><th>Conversión</th><th>Uso</th></tr></thead><tbody>{query.data.items.map((item) => { const version = item.versions[0]; return <tr key={item.id}><td><strong>{item.name}</strong><small>{item.key}</small></td><td><StatusBadge value={item.status} /></td><td>v{item.currentVersion}<small>{item.featureFlagKey}</small></td><td>{version?.enabledMarkets.join(', ') || '—'}</td><td>{version?.qualifyingEventType || '—'}<small>{version ? `${version.waitingPeriodHours} h de espera` : '—'}</small></td><td>{item._count.codes} códigos · {item._count.referrals} referrals</td></tr> })}</tbody></table></div><Pagination value={query.data.pagination} onChange={state.setPage} /></article>}</QueryState></>
}

function CodesPanel() {
  const state = usePage(['ACTIVE', 'REVOKED', 'EXPIRED']); const query = useQuery({ queryKey: ['f8-codes', state.query], queryFn: () => api(`/v1/admin/referrals/codes?${state.query}`, { schema: referralCodeListSchema }) })
...
function ReferralsPanel() {
  const state = usePage(['CREATED', 'QUALIFIED', 'CONVERTED', 'REJECTED', 'REVERSED']); const query = useQuery({ queryKey: ['f8-referrals', state.query], queryFn: () => api(`/v1/admin/referrals/referrals?${state.query}`, { schema: referralListSchema }) })
...
function ConversionsPanel() {
  const state = usePage(['QUALIFIED', 'CONVERTED', 'REVERSED']); const query = useQuery({ queryKey: ['f8-conversions', state.query], queryFn: () => api(`/v1/admin/referrals/conversions?${state.query}`, { schema: referralConversionListSchema }) })
...
function RewardsPanel() {
  const state = usePage(['PENDING', 'APPROVED', 'HELD', 'FULFILLED', 'REJECTED', 'REVERSED']); const query = useQuery({ queryKey: ['f8-rewards', state.query], queryFn: () => api(`/v1/admin/referrals/rewards?${state.query}`, { schema: referralRewardListSchema }) })
...
function DefinitionsPanel() {
  const state = usePage(['DRAFT', 'ACTIVE', 'PAUSED', 'ARCHIVED']); const query = useQuery({ queryKey: ['f8-definitions', state.query], queryFn: () => api(`/v1/admin/automation/definitions?${state.query}`, { schema: automationDefinitionListSchema }) })
