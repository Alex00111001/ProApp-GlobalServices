...
function responseCatalog(routes) {
  ...
  for (const route of routes) {
    const bindings = route.middleware.filter((item) => item.startsWith('responseContract(') || item.startsWith('errorResponseContract('));
    if (bindings.length > 1) throw new Error(`Multiple response authorities for ${route.method} ${route.path}.`);
    if (!bindings.length) continue;
    const expression = bindings[0].match(/^(?:responseContract|errorResponseContract)\(([A-Za-z_$][\w$]*)\.([A-Za-z_$][\w$]*)\)$/);
    ...
    if (!imported?.target.includes('contracts/')) throw new Error(`Response authority must come from a contract module for ${route.method} ${route.path}.`);
