...
Lifecycle is `DRAFT -> IN_REVIEW -> APPROVED -> SCHEDULED|PUBLISHED -> RETIRED`. Rejection is terminal for that version and changes require a new version. Creator and approver differ. Publishing re-resolves the ACTIVE Market/current policy and revalidates locale, official geography, service/category references, schema, approval, canonical, indexability and quality inside the transaction.
...
Canonical URLs use `/{marketCode}/{locale}/{type}/{slug}` with normalized lower-case path segments and no internal IDs. Location pages additionally derive the slug from an ACTIVE official division; arbitrary free-text locations are rejected. A published slug rename creates an explicit same-origin relative redirect. Redirect targets cannot include a scheme, authority, credentials, traversal, fragment, secret-bearing query or a loop. Missing content returns 404; retired content returns 410 unless an approved redirect exists.
