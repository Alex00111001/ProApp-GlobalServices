...
`totalPrice` remains the customer payment total for API compatibility. New quotes calculate all amounts in integer minor units and persist a pricing snapshot. While the Market Policy capability is disabled, commercial percentages are read centrally from `CLIENT_PLATFORM_FEE_PERCENTAGE` and `PROFESSIONAL_COMMISSION_PERCENTAGE`; the customer fee defaults to zero, preserving current customer totals until explicitly enabled.

When the Market Policy capability is enabled, booking creation no longer falls back to those global values. It requires the client's active market, the reviewed current `MarketPolicyVersion`, matching market/policy currency, and an effective `PricingPolicy` for that market. The closed pricing-rule contract is `{ clientPlatformFeeBasisPoints, professionalCommissionBasisPoints }`, with both values expressed as integers from 0 through 10,000. The booking stores the pricing-policy relation plus market-policy version/digest, pricing-policy version/rules digest, exact basis points, currency, and calculated minor-unit quote. Missing, malformed, expired, or currency-mismatched policy blocks booking creation rather than silently using a different market's defaults.

The professional lifecycle is `PENDING -> CONFIRMED -> IN_PROGRESS -> COMPLETED`, with explicit professional rejection from `PENDING` to the compatible `CANCELLED` projection. Confirm, reject, start, and complete use conditional database transitions; replay of the already-reached target state is idempotent and stale or skipped transitions return a conflict. `CONFIRMED` remains the persisted compatibility spelling for an accepted booking. Rejection has its own endpoint, event and audit action so it is not semantically confused with cancellation after acceptance.
