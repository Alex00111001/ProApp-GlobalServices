...
3. CI debe ejecutar el commit exacto entregado al cierre de cada bloque coherente; un CI histórico no
   ...
   disparan Platform verification completa; el cierre publica la referencia temporal
   `ci/closure/<sha-corto>` sobre el SHA final o usa CI automático del PR hacia `main`. La referencia no
   introduce cambios adicionales y activa Platform verification una sola vez.
...
migración, cambio de estado financiero, activación de Market, proveedor ni producción.
